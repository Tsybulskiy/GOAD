...
if ($gpo_exist) {
    ...
    #Remove-GPLink -Name "StarkWallpaper" -Target "DC=testers,DC=programmers,DC=kzi" -erroraction 'silentlycontinue'
} else {
    New-GPO -Name "StarkWallpaper" -comment "Change Wallpaper"
    New-GPLink -Name "StarkWallpaper" -Target "DC=testers,DC=programmers,DC=kzi"
    ...
    Set-GPRegistryValue -Name "StarkWallpaper" -Key "HKEY_kzi_MACHINE\Software\Policies\Microsoft\Windows NT\CurrentVersion\WinLogon" -ValueName SyncForegroundPolicy -Type DWORD -Value 1
